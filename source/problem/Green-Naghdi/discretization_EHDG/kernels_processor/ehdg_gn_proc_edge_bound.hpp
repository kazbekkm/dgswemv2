...
void Problem::local_dc_edge_boundary_kernel(const ESSPRKStepper& stepper, EdgeBoundaryType& edge_bound) {
    if (edge_bound.boundary.data.wet_dry_state.wet /*&& edge_bound.boundary.data.source.dispersive_correction*/) {
        ...
        auto& wd_state = edge_bound.boundary.data.wet_dry_state;
        auto& sl_state = edge_bound.boundary.data.slope_limit_state;

        sl_state.wet_neigh[edge_bound.boundary.bound_id] = wd_state.wet;
