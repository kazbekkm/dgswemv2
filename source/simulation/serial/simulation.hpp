...
void Simulation<ProblemType>::Run() {
    ...
    if (this->writer.Deserializing()) {
        this->writer.Deserialize(this->discretization.mesh);
    }
    ...
    if (this->writer.Serializing()) {
        this->writer.Serialize(this->discretization.mesh);
    }
}
...
void Simulation<ProblemType>::ComputeL2Residual() {
    ...
    std::cout << "hc L2 error: " << std::setprecision(15) << std::sqrt(hc_residual_L2) << std::endl;
}
